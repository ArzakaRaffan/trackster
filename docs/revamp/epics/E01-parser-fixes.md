...
**Acceptance:** ✅ backfill bersih, 0 deskripsi CSS, 0 transaksi Flip dobel. Saldo BCA final sudah dikoreksi manual oleh Arzaka sendiri (via `correctBalance`) setelah bug di bawah ditemukan — dianggap closed.

**⚠️ Bug ditemukan setelah S3 "selesai" (2026-09-24, sesi yang sama) — SUDAH DIPERBAIKI:** `TransactionService.remove()` (dipakai tombol Hapus UI yang baru di-ship) TIDAK punya guard baseline saldo seperti `createFromParsed()` — selalu `adjustBalance(+amount)` unconditional. User pakai tombol Hapus di 2 transaksi VA Tokopedia lama (id 194 Rp1.616.620, id 196 Rp1.455.400, occurredAt 4 Sep — sebelum baseline 22 Sep), saldo BCA sempat salah +Rp3.072.020. Arzaka koreksi saldo manual sendiri. Fix kode: `remove()` sekarang pakai `getLastManualAdjustmentAt` + `shouldAdjustBalance()` yang sama seperti `createFromParsed()` (commit `e5f0e0c`). Detail di [Gotchas.md](../../context/Gotchas.md).
